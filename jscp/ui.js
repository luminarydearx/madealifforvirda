...
function startHeartEffect() {
    ...
    requestAnimationFrame(() => {
        ...
        // Tampilkan pesan samping (kiri & kanan) sinkron dengan heart collage
        setTimeout(() => {
            showSideMessages();
        }, 800);
    });
...
/**
 * Tampilkan pesan teks di sisi kiri & kanan heart photo collage.
 * Warna sinkron dengan tema pink (matrixColor1 / matrixColor2 / sequenceColor).
 */
function showSideMessages() {
    const currentSettings = window.settings || settings;
    const leftEl = document.getElementById('sideMessageLeft');
    const rightEl = document.getElementById('sideMessageRight');

    if (!leftEl || !rightEl) return;

    // Ambil warna dari settings supaya selalu sinkron dengan tema yang dipilih
    const color1 = (currentSettings && currentSettings.matrixColor1) || '#ff69b4';
    const color2 = (currentSettings && currentSettings.matrixColor2) || '#ff1493';
    const sequenceColor = (currentSettings && currentSettings.sequenceColor) || '#ff69b4';

    // Terapkan warna secara dinamis (override CSS) ke kedua pesan
    [leftEl, rightEl].forEach(el => {
        el.style.color = '#ffffff';
        el.style.borderColor = hexToRgba(color1, 0.55);
        el.style.background = `linear-gradient(135deg, ${hexToRgba(color1, 0.18)}, ${hexToRgba(color2, 0.12)})`;
        el.style.textShadow =
            `0 0 8px ${color1}, 0 0 16px ${color2}, 0 0 24px ${hexToRgba(color2, 0.7)}`;
        el.style.boxShadow =
            `0 0 22px ${hexToRgba(color1, 0.55)}, ` +
            `0 0 45px ${hexToRgba(color2, 0.35)}, ` +
            `inset 0 0 18px ${hexToRgba(color1, 0.15)}`;
    });

    // Pakai sequenceColor untuk aksen tambahan
    leftEl.style.borderTopColor = hexToRgba(sequenceColor, 0.7);
    rightEl.style.borderTopColor = hexToRgba(sequenceColor, 0.7);
...
/**
 * Sembunyikan pesan samping (dipanggil saat reset / cleanup).
 */
...
/**
 * Helper: konversi hex (#rrggbb) ke rgba string dengan alpha.
 * Fallback kalau input bukan format hex yang valid.
 */
function hexToRgba(hex, alpha) {
    if (typeof hex !== 'string') return `rgba(255, 105, 180, ${alpha})`;
